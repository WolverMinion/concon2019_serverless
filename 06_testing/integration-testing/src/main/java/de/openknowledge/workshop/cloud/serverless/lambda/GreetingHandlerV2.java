...
public class GreetingHandlerV2 implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent requestEvent, Context context) {
        ...
        try {
            ...
            ErrorResponse errorResponse = new ErrorResponse(400, errorMissingParameter);
            responseEvent.setBody(gson.toJson(errorResponse));
     ...
     * @return
     */
    private GreetingResponse handleGreetingRequest(GreetingRequest name, Context context) {
        ...
        logger.log(String.format("INPUT firstName: %s", firstName));
        logger.log(String.format("INPUT lastName: %s", lastName));

        String greeting = GreetingService.greet(firstName, lastName);

        logger.log(String.format("OUTPUT greeting: %s" ,greeting));
